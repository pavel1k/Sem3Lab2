...
void merge(Sequence<T>* seq, int (*cmp)(T, T), int left, int mid, int right) {
    int i, j, k;
    int left_leng = mid - left + 1;       // äëèíû äâóõ âðåìåííûõ ñèêâåíñîâ
    int right_leng = right - mid;


    ArraySequence<T> first_tmp(left_leng), second_tmp(right_leng);               // ñîçäàäèì âðåìåííûå ñèêâåíñû
    Sequence<T>* left_seq = &first_tmp;
    Sequence<T>* right_seq = &second_tmp;

    for (i = 0; i < left_leng; i++) {                   // ñêîïèðóåì â íèõ çíà÷åíèÿ
    ...
    i = 0; j = 0; k = left;          // íà÷èíàåì ñëèÿíèå

    while (i < left_leng && j < right_leng) {             // ïîêà ìû íå ïåðåïèøåì ïîëíîñòüþ îäíó èç ïîëîâèí - ðàáîòàåì
        if (cmp(right_seq->Get(j), left_seq->Get(i))) {           // ñâåðÿåì ëåâóþ
            seq->Set(k, left_seq->Get(i));
            i++;
        }
        else {
            seq->Set(k, right_seq->Get(j));            // ñâåðÿåì è ïåðåïèñûâàåì(åñëè íàäî) ïðàâóþ
            j++;
        }
        k++;
    }                                      // ïåðåïèñàëàñü îäíà ïîëîâèíà 

    while (i < left_leng) {              // îñòàòêè èëè îò ëåâîãî èëè îò ïðàâîãî äîêèäûâàåì
        seq->Set(k, left_seq->Get(i));
        i++;
        k++;
    }
...
void merge_sort(Sequence<T>* seq, int (*cmp)(T, T), int left, int right) {
    if (left < right) { // (l + r) / 2 == l + (r - l)/ 2 çàòî áåç ïðîáëåì
        int mid = left + (right - left) / 2;  // äåëèì ïîïîëàì

        merge_sort(seq, cmp, left, mid);
        merge_sort(seq, cmp, mid + 1, right);    //ïðîäîëæàåì 

        merge(seq, cmp, left, mid, right);         // ïîòèõîíüêó ìåðäæèì
    }
}
